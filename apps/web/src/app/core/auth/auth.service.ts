...
import { AuthResponse, AuthTokens, AuthUser, DevicePlatform, OtpChannel, RequestOtpResponse } from '@gusto/contracts';
...
export class AuthService {
    ...
    requestOtp(phone: string, channel: OtpChannel = 'sms'): Promise<RequestOtpResponse> {
        return firstValueFrom(this.http.post<RequestOtpResponse>(`${this.base}/auth/otp/request`, { phone, channel }));
    }
    ...
    async logout(): Promise<void> {
        ...
        if (token) {
            await firstValueFrom(this.http.post(`${this.base}/auth/logout`, { refreshToken: token })).catch(
                () => void 0,
            );
        }
        this.state.set(null);
