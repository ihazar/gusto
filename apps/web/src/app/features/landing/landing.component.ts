...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <section class="hero">
        <div class="intro">
          <div class="brand">
            <span class="logo">🍳</span><span class="word">Gusto</span>
          </div>
          <h1>Home-cooked meals from chefs near you.</h1>
          <p class="lede">
            Real food, made by real people in their own kitchens — and brought
            to your door by your courier, <strong>Gus</strong>.
          </p>
          <ul class="points">
        ...
        <div class="card">
          @if (step() === 'phone') {
            ...
            <div class="seg" role="group" aria-label="Delivery channel">
              <button type="button" [class.active]="channel() === 'sms'" (click)="channel.set('sms')">SMS</button>
      ...
      <section class="cuisines">
        ...
        <div class="chips">
          @for (c of cuisines; track c) { <span class="chip">{{ c }}</span> }
        </div>
      </section>
  ...
  styles: [
    `
      :host { display: block; }
      .page { background: #faf7f2; color: #1d1b16; }

      .hero {
        display: grid; grid-template-columns: 1.1fr 0.9fr; gap: 40px; align-items: center;
        padding: 6vh 7vw; box-sizing: border-box;
        background:
      ...
      .logo { font-size: 32px; }
      .word { font-size: 28px; font-weight: 800; color: var(--gusto); letter-spacing: -0.5px; }
      ...
      .card {
        justify-self: end; width: 100%; max-width: 380px; background: #fff; border-radius: 20px;
        padding: 30px; box-shadow: 0 18px 50px rgba(120, 60, 30, 0.14);
      ...
      .primary { margin-top: 4px; padding: 13px; border: 0; border-radius: 12px; background: var(--gusto); color: #fff; font-weight: 700; font-size: 16px; cursor: pointer; }
      ...
      .band-title { font-size: clamp(22px, 2.6vw, 30px); text-align: center; margin: 0 0 28px; letter-spacing: -0.5px; }
      .band { padding: 8vh 7vw; }
      .steps { display: grid; grid-template-columns: repeat(3, 1fr); gap: 22px; max-width: 1000px; margin: 0 auto; }
      .steps article { background: #fff; border: 1px solid #f0e9dd; border-radius: 16px; padding: 26px; }
      .step-ico { font-size: 30px; }
      .steps h4 { margin: 14px 0 8px; font-size: 18px; }
      .steps p { margin: 0; color: #6b6457; font-size: 15px; line-height: 1.5; }

      .cuisines { padding: 2vh 7vw 8vh; text-align: center; }
      .cuisines-sub { color: #6b6457; margin: 0 0 22px; }
      .chips { display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; max-width: 760px; margin: 0 auto; }
      .chip { background: #fff; border: 1px solid #ecdfd0; color: #7a4a36; border-radius: 999px; padding: 9px 16px; font-weight: 600; font-size: 14px; }

      .callout {
        text-align: center; padding: 9vh 7vw; color: #fff;
        background: linear-gradient(120deg, #d2553a, #e0764f);
      }
      .callout h3 { font-size: clamp(24px, 3vw, 38px); margin: 0 0 10px; letter-spacing: -0.5px; }
      .callout p { margin: 0; opacity: .92; font-size: 17px; }

      .foot { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px; padding: 28px 7vw; }
      .muted { color: #8a8275; font-size: 14px; }

      @media (max-width: 860px) {
        .hero { grid-template-columns: 1fr; padding: 5vh 6vw; }
...
export class LandingComponent {
  ...
  readonly cuisines = [
    'Levantine', 'Georgian', 'Yemenite', 'Persian', 'Ethiopian',
